...
export const registerForPushNotificationsAsync = async () => {
  ...
  if (Device.isDevice) {
    ...
    try {
      // Make sure projectId is available - use your actual project ID as fallback
      const projectId =
        Constants.expoConfig?.extra?.eas?.projectId ||
        "2c80fd4d-5123-44f4-b3f3-651413aa9cfc";

      if (!projectId) {
        console.error("Missing projectId in app configuration");
        Alert.alert(
          "Configuration Error",
          "Missing projectId for push notifications"
        );
        return null;
      }
      ...
      //   "Could not generate push notification token: " + error.message
      // );
      return null;
    }
  } else {
...
export const updateHospitalPushToken = async (hospitalId: string) => {
  try {
    ...
    return {
      success: false,
      message: "Exception occurred while updating token",
    };
  }
};
  ...
  try {
    ...
    if (result && result.data) {
      successCount = result.data.filter(
        (ticket) => !ticket.error && ticket.status === "ok"
      ).length;
    }
...
const recordNotificationHistory = async (
  alertId,
  hospitals,
  notificationStatus
) => {
...
const sendSOSAlerts = async (alertData) => {
  try {
    // Get the 5 nearest hospitals
    const nearestHospitals = await getNearestHospitals(
      alertData.latitude,
      alertData.longitude,
      5
    );
    ...
    console.log(
      `Sending notifications to ${nearestHospitals.length} hospitals:`,
      nearestHospitals.map((h) => h.name)
    );
    const notificationStatus = await sendSOSNotifications(
      nearestHospitals,
      alertData
    );
    ...
    const hospitalList = nearestHospitals
      .map((h) => `- ${h.name} (${h.distance}km away)`)
      .join("\n");
...
// Exported functions
export { setupNotifications, sendSOSAlerts };
